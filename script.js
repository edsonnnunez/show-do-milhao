...
if (window.innerWidth > 768) {
    onValue(ref(database, 'game/countdown'), (snapshot) => {
        ...
        if (countdown !== null) {
            ...
            if (countdown > 0) {
                ...
                setTimeout(() => {
                    update(ref(database, 'game'), { countdown: countdown - 1 });
                }, 1000);
            } else {
                checkAnswers();
            }
...
document.querySelectorAll('.mobile-option-btn').forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        if (playerName && gameStatus === 'playing') {
            set(ref(database, `game/answers/${playerName}`), {
...
function checkAnswers(skip = false) {
    ...
    get(child(ref(database, 'game/answers'), '')).then((snapshot) => {
        ...
        if (!skip) {
            for (const playerName in answers) {
                ...
                if (playerAnswer === currentQuestion.ans) {
                    ...
                    update(ref(database, `game/players/${playerName}`), { score: score });
                }
            }
        }
        ...
        setTimeout(() => {
            ...
            remove(ref(database, 'game/answers'));
            ...
            if (endGame) {
                ...
                soundWinRound.play();
                update(gameRef, { currentQuestion: nextQuestionIndex, status: 'between-rounds', countdown: null });
            } else {
                update(gameRef, { currentQuestion: nextQuestionIndex, countdown: 15 });
            }
        }, 3000);
    });
}
